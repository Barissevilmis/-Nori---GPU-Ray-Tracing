...
void Canvas::draw(NVGcontext *ctx) {
    ...
    VectorGPU2i fbsize = m_size;
    VectorGPU2i offset = absolute_position();
    if (m_draw_border)
        fbsize -= 2;

#if defined(NANOGUI_USE_OPENGL) || defined(NANOGUI_USE_GLES)
    if (m_render_to_texture)
        offset = VectorGPU2i(offset.x(), scr->size().y() - offset.y() - m_size.y());
#endif

    if (m_draw_border)
        offset += VectorGPU2i(1, 1);

    fbsize = VectorGPU2i(VectorGPU2f(fbsize) * pixel_ratio);
    offset = VectorGPU2i(VectorGPU2f(offset) * pixel_ratio);
...
#endif
        rp->blit_to(VectorGPU2i(0, 0), fbsize, scr, offset);
    }
}
