...
VectorGPU2i Window::preferred_size(NVGcontext *ctx) const {
    if (m_button_panel)
        m_button_panel->set_visible(false);
    VectorGPU2i result = Widget::preferred_size(ctx);
    if (m_button_panel)
...
void Window::perform_layout(NVGcontext *ctx) {
    if (!m_button_panel) {
        ...
        for (auto w : m_button_panel->children()) {
            w->set_fixed_size(VectorGPU2i(22, 22));
            w->set_font_size(15);
        }
        m_button_panel->set_visible(true);
        m_button_panel->set_size(VectorGPU2i(width(), 22));
        m_button_panel->set_position(VectorGPU2i(
...
bool Window::mouse_enter_event(const VectorGPU2i &p, bool enter) {
    Widget::mouse_enter_event(p, enter);
    return true;
}

bool Window::mouse_drag_event(const VectorGPU2i &, const VectorGPU2i &rel,
                            int button, int /* modifiers */) {
    if (m_drag && (button & (1 << GLFW_MOUSE_BUTTON_1)) != 0) {
        m_pos += rel;
        m_pos = max(m_pos, VectorGPU2i(0));
...
bool Window::mouse_button_event(const VectorGPU2i &p, int button, bool down, int modifiers) {
...
bool Window::scroll_event(const VectorGPU2i &p, const VectorGPU2f &rel) {
