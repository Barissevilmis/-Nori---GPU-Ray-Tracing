...
Popup::Popup(Widget *parent, Window *parent_window)
    : Window(parent, ""), m_parent_window(parent_window), m_anchor_pos(VectorGPU2i(0)),
...
void Popup::perform_layout(NVGcontext *ctx) {
    if (m_layout || m_children.size() != 1) {
        ...
        m_children[0]->set_position(VectorGPU2i(0));
...
void Popup::refresh_relative_placement() {
    ...
    m_pos = m_parent_window->position() + m_anchor_pos - VectorGPU2i(0, m_anchor_offset);
}
...
void Popup::draw(NVGcontext* ctx) {
    ...
    VectorGPU2i base = m_pos + VectorGPU2i(0, m_anchor_offset);
    int sign = -1;
    if (m_side == Side::Left) {
        base.x() += m_size.x();
        sign = 1;
