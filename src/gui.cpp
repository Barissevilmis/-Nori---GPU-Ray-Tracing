...
namespace nori{
    ...
    : nanogui::Screen(nanogui::VectorGPU2i(block.getSize().x(), block.getSize().y() + 36),
                    "Nori", false),
    m_block(block) {
        ...
        panel->set_size(nanogui::VectorGPU2i(block.getSize().x(), block.getSize().y()));
        perform_layout();

        panel->set_position(
            nanogui::VectorGPU2i((m_size.x() - panel->size().x()) / 2, block.getSize().y()));
        ...
        const VectorGPU2i &size = m_block.getSize();
        m_shader->set_uniform("size", nanogui::VectorGPU2i(size.x(), size.y()));
        ...
        m_texture = new Texture(
            ...
            nanogui::VectorGPU2i(size.x() + 2 * m_block.getBorderSize(),
    ...
    void NoriScreen::draw_contents() {
        ...
        m_renderPass->begin();
        m_renderPass->set_viewport(nanogui::VectorGPU2i(0, 0),
                                nanogui::VectorGPU2i(m_pixel_ratio * size[0],
        ...
        m_shader->end();
        m_renderPass->set_viewport(nanogui::VectorGPU2i(0, 0), framebuffer_size());
        m_renderPass->end();
        m_block.unlock();
    }
