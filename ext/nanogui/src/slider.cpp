...
VectorGPU2i Slider::preferred_size(NVGcontext *) const {
    return VectorGPU2i(70, 16);
}

bool Slider::mouse_drag_event(const VectorGPU2i &p, const VectorGPU2i & /* rel */,
...
bool Slider::mouse_button_event(const VectorGPU2i &p, int /* button */, bool down, int /* modifiers */) {
    if (!m_enabled)
        return false;
...
void Slider::draw(NVGcontext* ctx) {
    ...
    VectorGPU2f knob_pos(start_x + (m_value - m_range.first) /
