...
public:
    ...
    const VectorGPU2i &position() const { return m_pos; }
    /// Set the position relative to the parent widget
    void set_position(const VectorGPU2i &pos) { m_pos = pos; }

    /// Return the absolute position on screen
    VectorGPU2i absolute_position() const {
        return m_parent ?
            (parent()->absolute_position() + m_pos) : m_pos;
    }

    /// Return the size of the widget
    const VectorGPU2i &size() const { return m_size; }
    /// set the size of the widget
    void set_size(const VectorGPU2i &size) { m_size = size; }
     ...
     */
    void set_fixed_size(const VectorGPU2i &fixed_size) { m_fixed_size = fixed_size; }

    /// Return the fixed size (see \ref set_fixed_size())
    const VectorGPU2i &fixed_size() const { return m_fixed_size; }
    ...
    bool contains(const VectorGPU2i &p) const {
        VectorGPU2i d = p - m_pos;
        return d.x() >= 0 && d.y() >= 0 &&
               d.x() < m_size.x() && d.y() < m_size.y();
    }

    /// Determine the widget located at the given position value (recursive)
    Widget *find_widget(const VectorGPU2i &p);
    const Widget *find_widget(const VectorGPU2i &p) const;

    /// Handle a mouse button event (default implementation: propagate to children)
    virtual bool mouse_button_event(const VectorGPU2i &p, int button, bool down, int modifiers);

    /// Handle a mouse motion event (default implementation: propagate to children)
    virtual bool mouse_motion_event(const VectorGPU2i &p, const VectorGPU2i &rel, int button, int modifiers);

    /// Handle a mouse drag event (default implementation: do nothing)
    virtual bool mouse_drag_event(const VectorGPU2i &p, const VectorGPU2i &rel, int button, int modifiers);

    /// Handle a mouse enter/leave event (default implementation: record this fact, but do nothing)
    virtual bool mouse_enter_event(const VectorGPU2i &p, bool enter);

    /// Handle a mouse scroll event (default implementation: propagate to children)
    virtual bool scroll_event(const VectorGPU2i &p, const VectorGPU2f &rel);
    ...
    virtual VectorGPU2i preferred_size(NVGcontext *ctx) const;
...
protected:
    ...
    ref<Theme> m_theme;
    ref<Layout> m_layout;
    VectorGPU2i m_pos, m_size, m_fixed_size;
