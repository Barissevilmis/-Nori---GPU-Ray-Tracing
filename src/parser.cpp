        ...
        std::function<NoriObject *(pugi::xml_node &, PropertyList &, int)> parseTag = [&](
            ...
            try {
                if (currentIsObject) {
                    ...
                    //std::cout<<node.name()<<" "<<node.attribute("type").value()<<endl;
                    ...
                    switch (tag) {
                        ...
                        case EPoint: {
                                check_attributes(node, { "name", "value" });
                                list.setPoint(node.attribute("name").value(), PointGPU3f(toVector3f(node.attribute("value").value())));
                            }
                            break;
                        case EVector: {
                                check_attributes(node, { "name", "value" });
                                list.setVector(node.attribute("name").value(), VectorGPU3f(toVector3f(node.attribute("value").value())));
                            }
                            break;
                        case EColor: {
                                check_attributes(node, { "name", "value" });
                                list.setColor(node.attribute("name").value(), ColorGPU3f(toVector3f(node.attribute("value").value()).array()));
                            }
                            break;
                        case ETransform: {
                        ...
                        case ELookAt: {
                                ...
                                VectorGPU3f dir = (target - origin).normalized();
                                VectorGPU3f left = up.normalized().cross(dir).normalized();
                                VectorGPU3f newUp = dir.cross(left).normalized();
