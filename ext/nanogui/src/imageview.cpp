...
void ImageView::center() {
    if (!m_image)
        return;
    m_offset = VectorGPU2i(.5f * (VectorGPU2f(m_size) * screen()->pixel_ratio() - VectorGPU2f(m_image->size()) * scale()));
}
...
VectorGPU2f ImageView::pos_to_pixel(const VectorGPU2f &p) const {
    VectorGPU2f p2 = p;
    if (m_draw_border)
        p2 -= 1.f;
...
VectorGPU2f ImageView::pixel_to_pos(const VectorGPU2f &p) const {
    VectorGPU2i pos = (p * scale() + m_offset) / screen()->pixel_ratio();
    if (m_draw_border)
        pos += 1.f;
    return pos;
}
...
bool ImageView::mouse_drag_event(const VectorGPU2i & /* p */, const VectorGPU2i &rel,
...
bool ImageView::scroll_event(const VectorGPU2i &p, const VectorGPU2f &rel) {
    if (!m_enabled || !m_image)
        return false;

    VectorGPU2f p1 = pos_to_pixel(p - m_pos);
    m_scale += rel.y();
    ...
    VectorGPU2f p2 = pos_to_pixel(p - m_pos);
...
void ImageView::draw(NVGcontext *ctx) {
    ...
    VectorGPU2i top_left = VectorGPU2i(pixel_to_pos(VectorGPU2f(0.f, 0.f))),
             size     = VectorGPU2i(pixel_to_pos(VectorGPU2f(m_image->size())) - VectorGPU2f(top_left));
    ...
    if (scale() > 100 && m_pixel_callback) {
        ...
        VectorGPU2i start = max(VectorGPU2i(0), VectorGPU2i(pos_to_pixel(VectorGPU2f(0.f, 0.f))) - 1),
                 end   = min(VectorGPU2i(pos_to_pixel(VectorGPU2f(m_size))) + 1, m_image->size() - 1);
        ...
        for (int y = start.y(); y <= end.y(); ++y) {
            for (int x = start.x(); x <= end.x(); ++x) {
                VectorGPU2i pos = VectorGPU2i(pixel_to_pos(VectorGPU2f(x + .5f, y + .5f)));

                m_pixel_callback(VectorGPU2i(x, y), text, 20);
...
void ImageView::draw_contents() {
    ...
    m_offset = (VectorGPU2f(VectorGPU2i(m_offset / pixel_ratio)) * pixel_ratio);

    VectorGPU2f bound1 = VectorGPU2f(m_size) * pixel_ratio,
             bound2 = -VectorGPU2f(m_image->size()) * scale();
    ...
    VectorGPU2i viewport_size = render_pass()->viewport().second;

    float scale = std::pow(2.f, m_scale / 5.f);

    Matrix4f matrix_background =
        Matrix4f::scale(VectorGPU3f(m_image->size().x() * scale / 20.f,
                                 m_image->size().y() * scale / 20.f, 1.f));

    Matrix4f matrix_image =
        Matrix4f::ortho(0.f, viewport_size.x(), viewport_size.y(), 0.f, -1.f, 1.f) *
        Matrix4f::translate(VectorGPU3f(m_offset.x(), (int) m_offset.y(), 0.f)) *
        Matrix4f::scale(VectorGPU3f(m_image->size().x() * scale,
