...
void RenderPass::end() {
    ...
    CHK(glBindFramebuffer(GL_FRAMEBUFFER, 0));
    if (m_blit_target)
        blit_to(VectorGPU2i(0, 0), m_framebuffer_size, m_blit_target, VectorGPU2i(0, 0));
...
void RenderPass::resize(const VectorGPU2i &size) {
    for (size_t i = 0; i < m_targets.size(); ++i) {
        Texture *texture = dynamic_cast<Texture *>(m_targets[i].get());
        if (texture)
            texture->resize(size);
    ...
    m_viewport_offset = VectorGPU2i(0, 0);
...
void RenderPass::set_viewport(const VectorGPU2i &offset, const VectorGPU2i &size) {
    ...
    if (m_active) {
        ...
        if (m_viewport_offset == VectorGPU2i(0, 0) &&
            m_viewport_size == m_framebuffer_size)
            CHK(glDisable(GL_SCISSOR_TEST));
        else
            CHK(glEnable(GL_SCISSOR_TEST));
...
void RenderPass::blit_to(const VectorGPU2i &src_offset,
                         const VectorGPU2i &src_size,
                         Object *dst,
                         const VectorGPU2i &dst_offset) {
    ...
    VectorGPU2i src_end = src_offset + src_size,
