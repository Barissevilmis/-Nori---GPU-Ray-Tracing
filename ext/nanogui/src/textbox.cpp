...
TextBox::TextBox(Widget *parent, const std::string &value)
    : Widget(parent),
      ...
      m_cursor_pos(-1),
      m_selection_pos(-1),
      m_mouse_pos(VectorGPU2i(-1,-1)),
      m_mouse_down_pos(VectorGPU2i(-1,-1)),
      m_mouse_drag_pos(VectorGPU2i(-1,-1)),
...
VectorGPU2i TextBox::preferred_size(NVGcontext *ctx) const {
    VectorGPU2i size(0, font_size() * 1.4f);
...
void TextBox::draw(NVGcontext* ctx) {
    ...
    VectorGPU2i draw_pos(m_pos.x(), m_pos.y() + m_size.y() * 0.5f + 1);
    ...
    if (m_spinnable && !focused()) {
        ...
        /* up button */ {
            ...
            VectorGPU2f icon_pos(m_pos.x() + 4.f,
    ...
    VectorGPU2i old_draw_pos(draw_pos);
...
bool TextBox::mouse_enter_event(const VectorGPU2i &p, bool enter) {
    Widget::mouse_enter_event(p, enter);
    return true;
}

bool TextBox::mouse_button_event(const VectorGPU2i &p, int button, bool down,
                                 int modifiers) {
...
bool TextBox::mouse_motion_event(const VectorGPU2i &p, const VectorGPU2i & /* rel */,
...
bool TextBox::mouse_drag_event(const VectorGPU2i &p, const VectorGPU2i &/* rel */,
                               int /* button */, int /* modifiers */) {
    m_mouse_pos = p;
    m_mouse_drag_pos = p;
...
TextBox::SpinArea TextBox::spin_area(const VectorGPU2i & pos) {
