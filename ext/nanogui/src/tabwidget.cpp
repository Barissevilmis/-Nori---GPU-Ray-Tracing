...
VectorGPU2i TabWidgetBase::preferred_size(NVGcontext* ctx) const {
    nvgFontFace(ctx, m_font.c_str());
    nvgFontSize(ctx, font_size());
    nvgTextAlign(ctx, NVG_ALIGN_LEFT | NVG_ALIGN_TOP);
...
std::pair<int, bool> TabWidgetBase::tab_at_position(const VectorGPU2i &p, bool test_vertical) const {
    int tab_height = font_size() + 2 * m_theme->m_tab_button_vertical_padding;
...
bool TabWidgetBase::mouse_button_event(const VectorGPU2i &p, int button, bool down,
                                       int modifiers) {
    ...
    if (m_popup_callback && button == GLFW_MOUSE_BUTTON_2 && down && index != -1 &&
        ...
        m_popup->set_position(p + VectorGPU2i(8, -6));
        ...
        m_popup->set_size(m_popup->preferred_size(ctx) + VectorGPU2i(40, 0));
        ...
        if (index >= 0) {
            if (close && m_tab_drag_index == -1) {
                if (down) {
                    ...
                    mouse_motion_event(p, VectorGPU2i(0), 0, 0);
                }
            } else {
                if (down) {
...
bool TabWidgetBase::mouse_enter_event(const VectorGPU2i &/* p */, bool /* enter */) {
...
bool TabWidgetBase::mouse_motion_event(const VectorGPU2i &p, const VectorGPU2i &rel,
...
void TabWidget::perform_layout(NVGcontext* ctx) {
    ...
    for (Widget *child : m_children) {
        child->set_position(VectorGPU2i(m_padding, m_padding + tab_height + 1));
        child->set_size(m_size - VectorGPU2i(2*m_padding, 2*m_padding + tab_height + 1));
...
VectorGPU2i TabWidget::preferred_size(NVGcontext* ctx) const {
    VectorGPU2i base_size = TabWidgetBase::preferred_size(ctx),
             content_size = VectorGPU2i(0);
    for (Widget *child : m_children)
