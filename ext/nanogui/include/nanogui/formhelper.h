 ...
 *    h->add_window(VectorGPU2i(10,10),"Menu");
 *
 *    // Start a new group
 *    h->add_group("Group 1");
 *
...
public:
    ...
    Window *add_window(const VectorGPU2i &pos,
    ...
    add_variable(const std::string &label, const std::function<void(const Type &)> &setter,
        ...
        VectorGPU2i fs = widget->fixed_size();
        widget->set_fixed_size(VectorGPU2i(fs.x() != 0 ? fs.x() : m_fixed_size.x(),
    ...
    void set_fixed_size(const VectorGPU2i &fw) { m_fixed_size = fw; }

    /// The current fixed size being used for newly added widgets.
    VectorGPU2i fixed_size() { return m_fixed_size; }
...
protected:
    ...
    VectorGPU2i m_fixed_size = VectorGPU2i(0, 20);
