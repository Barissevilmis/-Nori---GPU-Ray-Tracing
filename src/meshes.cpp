...
namespace nori{
        ...
        Mesh::Mesh(const PropertyList &propList) {
            ...
            std::vector<VectorGPU3f>   positions;
            std::vector<VectorGPU2f>   texcoords;
            std::vector<VectorGPU3f>   normals;
            ...
            for(uint32_t i = 0; i < indices.size(); i+=3){
                v_F.push_back(VectorGPU3i(indices[i], indices[i+1], indices[i+2]));
            }
            for (uint32_t i=0; i<vertices.size(); ++i)
            {
                m_V.col(i) = positions[vertices[i].p-1];
                v_V.push_back(positions[vertices[i].p-1]);
            }
            if (!normals.empty()) {
                m_N.resize(3, vertices.size());
                N_exists = 1;
                for (uint32_t i=0; i<vertices.size(); ++i){   
                    m_N.col(i) = normals[vertices[i].p-1];
                    v_N.push_back(normals[vertices[i].p-1]);
                }
                std::cout<<v_N.size()<<endl;
            }
            else{
                N_exists = 0;
            }
            ...
            if (!texcoords.empty()) {
                ...
                UV_exists = 1;
                for (uint32_t i=0; i<vertices.size(); ++i){
                    m_UV.col(i) = texcoords[vertices[i].p-1];
                    v_UV.push_back(texcoords[vertices[i].p-1]);
                }
