...
public:
    ...
    /// Set window size
    void set_size(const VectorGPU2i& size);

    /// Return the framebuffer size (potentially larger than size() on high-DPI screens)
    const VectorGPU2i &framebuffer_size() const { return m_fbsize; }
    ...
    virtual bool resize_event(const VectorGPU2i& size);

    /// Set the resize callback
    std::function<void(VectorGPU2i)> resize_callback() const { return m_resize_callback; }
    void set_resize_callback(const std::function<void(VectorGPU2i)> &callback) {
        m_resize_callback = callback;
    }

    /// Return the last observed mouse position value
    VectorGPU2i mouse_pos() const { return m_mouse_pos; }
...
protected:
    ...
    bool m_float_buffer;
    bool m_redraw;
    std::function<void(VectorGPU2i)> m_resize_callback;
#if defined(NANOGUI_USE_METAL)
