...
PopupButton::PopupButton(Widget *parent, const std::string &caption, int button_icon)
    ...
    m_popup->set_size(VectorGPU2i(320, 250));
    m_popup->set_visible(false);

    m_icon_extra_scale = 0.8f; // widget override
}

VectorGPU2i PopupButton::preferred_size(NVGcontext *ctx) const {
    return Button::preferred_size(ctx) + VectorGPU2i(15, 0);
}
...
void PopupButton::draw(NVGcontext* ctx) {
    ...
    if (m_chevron_icon) {
        ...
        VectorGPU2f icon_pos(0, m_pos.y() + m_size.y() * 0.5f - 1);
...
void PopupButton::perform_layout(NVGcontext *ctx) {
    ...
    if (parent_window) {
        ...
        if (m_popup->side() == Popup::Right)
            m_popup->set_anchor_pos(VectorGPU2i(parent_window->width() + anchor_size, pos_y));
        else
            m_popup->set_anchor_pos(VectorGPU2i(-anchor_size, pos_y));
    } else {
        m_popup->set_position(absolute_position() + VectorGPU2i(width() + anchor_size + 1,  m_size.y() / 2 - anchor_size));
    }
}
