...
void TextArea::append(const std::string &text) {
    ...
    do {
        ...
        if (*str == '\n') {
            m_offset = VectorGPU2i(0, m_offset.y() + font_size());
            m_max_size = max(m_max_size, m_offset);
        }
    } while (*str++ != 0);
...
bool TextArea::keyboard_event(int key, int /* scancode */, int action, int modifiers) {
    if (m_selectable && focused()) {
        if (key == GLFW_KEY_C && modifiers == SYSTEM_COMMAND_MOD && action == GLFW_PRESS &&
            m_selection_start != -1 && m_selection_end != -1) {
            VectorGPU2i start = m_selection_start, end = m_selection_end;
...
VectorGPU2i TextArea::preferred_size(NVGcontext *) const {
...
void TextArea::draw(NVGcontext *ctx) {
    ...
    VectorGPU2i selection_end = block_to_position(m_selection_end);
    selection_end += m_pos + m_padding;
    if (m_selection_end != VectorGPU2i(-1)) {
        nvgBeginPath(ctx);
    ...
    VectorGPU2i selection_start = block_to_position(m_selection_start);
    ...
    if (m_selection_end != VectorGPU2i(-1) && m_selection_end != VectorGPU2i(-1)) {
        nvgBeginPath(ctx);
    ...
    for (auto it = start_it; it != end_it; ++it) {
        ...
        VectorGPU2i offset = block.offset + m_pos + m_padding;
...
bool TextArea::mouse_button_event(const VectorGPU2i &p, int button, bool down,
                                  int /* modifiers */) {
...
bool TextArea::mouse_drag_event(const VectorGPU2i &p, const VectorGPU2i &/* rel */,
...
VectorGPU2i TextArea::position_to_block(const VectorGPU2i &pos) const {
    NVGcontext *ctx = screen()->nvg_context();
...
VectorGPU2i TextArea::block_to_position(const VectorGPU2i &pos) const {
    ...
    int nglyphs =
        nvgTextGlyphPositions(ctx, block.offset.x(), block.offset.y(),
                              block.text.c_str(), nullptr, glyphs, max_glyphs);
    if (pos.y() == nglyphs)
        return block.offset + VectorGPU2i(glyphs[pos.y() - 1].maxx + 1, 0);
    else if (pos.y() > nglyphs)
        return VectorGPU2i(-1, -1);

    return block.offset + VectorGPU2i(glyphs[pos.y()].x, 0);
}
