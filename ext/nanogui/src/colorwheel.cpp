...
VectorGPU2i ColorWheel::preferred_size(NVGcontext *) const {
    return { 100, 100 };
}
...
bool ColorWheel::mouse_button_event(const VectorGPU2i &p, int button, bool down,
                                  int modifiers) {
...
bool ColorWheel::mouse_drag_event(const VectorGPU2i &p, const VectorGPU2i &,
                                int, int) {
    return adjust_position(p, m_drag_region) != None;
}

ColorWheel::Region ColorWheel::adjust_position(const VectorGPU2i &p, Region considered_regions) {
    float x = p.x() - m_pos.x(),
          y = p.y() - m_pos.y(),
          w = m_size.x(),
          h = m_size.y();
    ...
    float a = -m_hue * 2 * NVG_PI,
          sin_a = std::sin(a),
          cos_a = std::cos(a);
    VectorGPU2f xy(cos_a * x - sin_a * y,
                sin_a * x + cos_a * y);
