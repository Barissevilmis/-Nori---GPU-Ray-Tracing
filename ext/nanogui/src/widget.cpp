...
VectorGPU2i Widget::preferred_size(NVGcontext *ctx) const {
    if (m_layout)
...
void Widget::perform_layout(NVGcontext *ctx) {
    if (m_layout) {
        ...
        for (auto c : m_children) {
            VectorGPU2i pref = c->preferred_size(ctx), fix = c->fixed_size();
            c->set_size(VectorGPU2i(
                fix[0] ? fix[0] : pref[0],
                fix[1] ? fix[1] : pref[1]
            ));
...
Widget *Widget::find_widget(const VectorGPU2i &p) {
...
const Widget *Widget::find_widget(const VectorGPU2i &p) const {
    for (auto it = m_children.rbegin(); it != m_children.rend(); ++it) {
        Widget *child = *it;
        if (child->visible() && child->contains(p - m_pos))
            return child->find_widget(p - m_pos);
...
bool Widget::mouse_button_event(const VectorGPU2i &p, int button, bool down, int modifiers) {
    for (auto it = m_children.rbegin(); it != m_children.rend(); ++it) {
        Widget *child = *it;
        if (child->visible() && child->contains(p - m_pos) &&
...
bool Widget::mouse_motion_event(const VectorGPU2i &p, const VectorGPU2i &rel, int button, int modifiers) {
    bool handled = false;
...
bool Widget::scroll_event(const VectorGPU2i &p, const VectorGPU2f &rel) {
    for (auto it = m_children.rbegin(); it != m_children.rend(); ++it) {
        Widget *child = *it;
        if (!child->visible())
            continue;
...
bool Widget::mouse_drag_event(const VectorGPU2i &, const VectorGPU2i &, int, int) {
    return false;
}

bool Widget::mouse_enter_event(const VectorGPU2i &, bool enter) {
    m_mouse_focus = enter;
    return false;
}
