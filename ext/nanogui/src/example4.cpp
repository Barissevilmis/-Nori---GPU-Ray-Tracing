...
using nanogui::VectorGPU3f;
using nanogui::VectorGPU2i;
...
public:
    ExampleApplication() : nanogui::Screen(VectorGPU2i(800, 600), "NanoGUI Test", false) {
        using namespace nanogui;
        ...
        b0->set_callback([this]() {
            m_canvas->set_background_color(
                VectorGPU4i(rand() % 256, rand() % 256, rand() % 256, 255));
        });
