...
VectorGPU2i BoxLayout::preferred_size(NVGcontext *ctx, const Widget *widget) const {
    VectorGPU2i size(2*m_margin);
    ...
    for (auto w : widget->children()) {
        ...
        VectorGPU2i ps = w->preferred_size(ctx), fs = w->fixed_size();
        VectorGPU2i target_size(
            fs[0] ? fs[0] : ps[0],
            fs[1] ? fs[1] : ps[1]
        );
    ...
    return size + VectorGPU2i(0, y_offset);
}

void BoxLayout::perform_layout(NVGcontext *ctx, Widget *widget) const {
    VectorGPU2i fs_w = widget->fixed_size();
    VectorGPU2i container_size(
...
VectorGPU2i GroupLayout::preferred_size(NVGcontext *ctx, const Widget *widget) const {
    ...
    for (auto c : widget->children()) {
        ...
        VectorGPU2i ps = c->preferred_size(ctx), fs = c->fixed_size();
        VectorGPU2i target_size(
            fs[0] ? fs[0] : ps[0],
            fs[1] ? fs[1] : ps[1]
        );
    ...
    height += m_margin;
    return VectorGPU2i(width, height);
}
...
void GroupLayout::perform_layout(NVGcontext *ctx, Widget *widget) const {
    ...
    for (auto c : widget->children()) {
        ...
        bool indent_cur = indent && label == nullptr;
        VectorGPU2i ps = VectorGPU2i(available_width - (indent_cur ? m_group_indent : 0),
                               c->preferred_size(ctx).y());
        VectorGPU2i fs = c->fixed_size();
        ...
        c->set_position(VectorGPU2i(m_margin + (indent_cur ? m_group_indent : 0), height));
...
VectorGPU2i GridLayout::preferred_size(NVGcontext *ctx,
...
void GridLayout::compute_layout(NVGcontext *ctx, const Widget *widget, std::vector<int> *grid) const {
    ...
    for (int i2 = 0; i2 < dim[axis2]; i2++) {
        for (int i1 = 0; i1 < dim[axis1]; i1++) {
            ...
            VectorGPU2i ps = w->preferred_size(ctx);
            VectorGPU2i fs = w->fixed_size();
            VectorGPU2i target_size(
                fs[0] ? fs[0] : ps[0],
                fs[1] ? fs[1] : ps[1]
            );
...
void GridLayout::perform_layout(NVGcontext *ctx, Widget *widget) const {
    ...
    int axis1 = (int) m_orientation, axis2 = (axis1 + 1) % 2;
    VectorGPU2i start = m_margin + extra;

    size_t num_children = widget->children().size();
    size_t child = 0;

    VectorGPU2i pos = start;
    for (int i2 = 0; i2 < dim[axis2]; i2++) {
...
VectorGPU2i AdvancedGridLayout::preferred_size(NVGcontext *ctx, const Widget *widget) const {
    /* Compute minimum row / column sizes */
    std::vector<int> grid[2];
    compute_layout(ctx, widget, grid);
...
void AdvancedGridLayout::perform_layout(NVGcontext *ctx, Widget *widget) const {
    ...
    for (int axis=0; axis<2; ++axis) {
        ...
        for (Widget *w : widget->children()) {
            ...
            VectorGPU2i pos = w->position(), size = w->size();
            pos[axis] = item_pos;
