...
void VScrollPanel::perform_layout(NVGcontext *ctx) {
    ...
    if (m_child_preferred_height > m_size.y()) {
        child->set_position(VectorGPU2i(0, -m_scroll * (m_child_preferred_height - m_size.y())));
        child->set_size(VectorGPU2i(m_size.x() - 12, m_child_preferred_height));
    } else {
        child->set_position(VectorGPU2i(0));
...
VectorGPU2i VScrollPanel::preferred_size(NVGcontext *ctx) const {
    if (m_children.empty())
        return VectorGPU2i(0);
    return m_children[0]->preferred_size(ctx) + VectorGPU2i(12, 0);
}

bool VScrollPanel::mouse_drag_event(const VectorGPU2i &p, const VectorGPU2i &rel,
...
bool VScrollPanel::mouse_button_event(const VectorGPU2i &p, int button, bool down,
                                      int modifiers) {
    ...
    if (down && button == GLFW_MOUSE_BUTTON_1 && !m_children.empty() &&
        ...
        m_children[0]->set_position(
            VectorGPU2i(0, -m_scroll * (m_child_preferred_height - m_size.y())));
        m_update_layout = true;
        return true;
    }
    return false;
...
bool VScrollPanel::scroll_event(const VectorGPU2i &p, const VectorGPU2f &rel) {
    if (!m_children.empty() && m_child_preferred_height > m_size.y()) {
        ...
        VectorGPU2i old_pos = child->position();
        child->set_position(VectorGPU2i(0, -m_scroll*(m_child_preferred_height - m_size.y())));
        VectorGPU2i new_pos = child->position();
        m_update_layout = true;
...
void VScrollPanel::draw(NVGcontext *ctx) {
    ...
    child->set_position(VectorGPU2i(0, yoffset));
    m_child_preferred_height = child->preferred_size(ctx).y();
    float scrollh = height() *
        std::min(1.f, height() / (float) m_child_preferred_height);
