...
public:
    using PixelCallback = std::function<void(const VectorGPU2i &, char **, size_t)>;
    ...
    VectorGPU2f offset() const { return m_offset; }
    /// Set the pixel offset of the zoomed image rectangle
    void set_offset(const VectorGPU2f &offset) { m_offset = offset; }
    ...
    VectorGPU2f pos_to_pixel(const VectorGPU2f &p) const;
    /// Convert a pixel position in the image to a position within the widget
    VectorGPU2f pixel_to_pos(const VectorGPU2f &p) const;
