...
VectorGPU2i ImagePanel::grid_size() const {
    ...
    return VectorGPU2i(n_cols, n_rows);
}

int ImagePanel::index_for_position(const VectorGPU2i &p) const {
    VectorGPU2f pp = (VectorGPU2f(p - m_pos) - VectorGPU2f(m_margin)) /
                  (float)(m_thumb_size + m_spacing);
    ...
    VectorGPU2i grid_pos(pp), grid = grid_size();
...
bool ImagePanel::mouse_motion_event(const VectorGPU2i &p, const VectorGPU2i & /* rel */,
                                    int /* button */, int /* modifiers */) {
    m_mouse_index = index_for_position(p);
    return true;
}

bool ImagePanel::mouse_button_event(const VectorGPU2i &p, int /* button */, bool down,
                                    int /* modifiers */) {
...
VectorGPU2i ImagePanel::preferred_size(NVGcontext *) const {
    VectorGPU2i grid = grid_size();
    return VectorGPU2i(
...
void ImagePanel::draw(NVGcontext* ctx) {
    ...
    for (size_t i = 0; i < m_images.size(); ++i) {
        VectorGPU2i p = m_pos + VectorGPU2i(m_margin) +
            VectorGPU2i((int) i % grid.x(), (int) i / grid.x()) * (m_thumb_size + m_spacing);
        int imgw, imgh;
