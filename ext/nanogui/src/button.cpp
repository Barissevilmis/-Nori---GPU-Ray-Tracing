...
VectorGPU2i Button::preferred_size(NVGcontext *ctx) const {
    ...
    return VectorGPU2i((int)(tw + iw) + 20, font_size + 10);
}

bool Button::mouse_enter_event(const VectorGPU2i &p, bool enter) {
    Widget::mouse_enter_event(p, enter);
    return true;
}

bool Button::mouse_button_event(const VectorGPU2i &p, int button, bool down, int modifiers) {
...
void Button::draw(NVGcontext *ctx) {
    ...
    VectorGPU2f center = VectorGPU2f(m_pos) + VectorGPU2f(m_size) * 0.5f;
    VectorGPU2f text_pos(center.x() - tw * 0.5f, center.y() - 1);
    NVGcolor text_color =
