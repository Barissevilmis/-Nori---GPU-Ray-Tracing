...
static EM_BOOL nanogui_emscripten_resize_callback(int eventType, const EmscriptenUiEvent *, void *) {
    ...
    for (auto it: __nanogui_screens) {
        ...
        screen->resize_event(VectorGPU2i((int) w2, (int) h2));
        screen->redraw();
    }
...
Screen::Screen(const VectorGPU2i &size, const std::string &caption, bool resizable,
...
void Screen::initialize(GLFWwindow *window, bool shutdown_glfw) {
    ...
    m_fbsize = VectorGPU2i((int) w2, (int) h2);
    m_size = VectorGPU2i((int) w, (int) h);
...
void Screen::set_size(const VectorGPU2i &size) {
...
#if defined(_WIN32) || defined(__linux__) || defined(EMSCRIPTEN)
    m_fbsize = m_size;
    m_size = VectorGPU2i(VectorGPU2f(m_size) / m_pixel_ratio);
#else
...
void Screen::draw_widgets() {
    ...
    if (elapsed > 0.5f) {
        ...
        if (widget && !widget->tooltip().empty()) {
            ...
            VectorGPU2i pos = widget->absolute_position() +
                           VectorGPU2i(widget->width() / 2, widget->height() + 10);
...
bool Screen::resize_event(const VectorGPU2i& size) {
...
void Screen::cursor_pos_callback_event(double x, double y) {
    VectorGPU2i p((int) x, (int) y);

#if defined(_WIN32) || defined(__linux__) || defined(EMSCRIPTEN)
    p = VectorGPU2i(VectorGPU2f(p) / m_pixel_ratio);
#endif
...
void Screen::scroll_callback_event(double x, double y) {
    ...
    try {
        ...
        m_redraw |= scroll_event(m_mouse_pos, VectorGPU2f(x, y));
    } catch (const std::exception &e) {
        std::cerr << "Caught exception in event handler: " << e.what() << std::endl;
    }
}
